...
import com.dongdian.shenquan.databinding.SearchResultFragmentBinding;
...
public class SearchResultFragment extends MyBaseFragment<SearchResultFragmentBinding,SearchResultFragmentViewModel> {
    ...
    public int initVariableId() {
        return BR.searchresultfragmentviewmodel;
    }
    @Override
    public void initData() {
        super.initData();
        Bundle arguments = getArguments();
        mall_id = arguments.getString("mall_id");
        searchKey = arguments.getString("searchKey");
        binding.searchResultFragmentRecycler.setEmptyView(R.layout.empty);
        binding.searchResultFragmentRecycler.setAdapter(adapter);
    ...
    private void initProduct() {
        ...
        Utils.initListView(getActivity(), binding.searchResultFragmentRecycler, new DividerDecoration(Color.parseColor("#dddddd"), 0), adapter, new PullListener() {
            @Override
            ...
            public void onLoadMore() {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        binding.searchResultFragmentRecycler.setRefreshing(false);
                        if (page == 1) {
                            return;
                        }
                        viewModel.getList(page);
