...
import com.dongdian.shenquan.databinding.ActivityPpggoodsListBinding;
...
public class PPGGoodsListActivity extends MyBaseActivity<ActivityPpggoodsListBinding,PPGGoodsListViewModel> {
    ...
    public int initContentView(Bundle savedInstanceState) {
        setbarfull();
        statusBarLightMode();
        return R.layout.activity_ppggoods_list;
    }

    @Override
    public int initVariableId() {
        return BR.ppggoodslistviewmodel;
    }
    ...
    public void initData() {
        ...
        binding.ppggoodsListRecycler.setEmptyView(R.layout.empty);
        binding.ppggoodsListRecycler.setAdapter(adapter);
        viewModel.id.set(id);
        initProduct();
    }
    //列表初始化
    private void initProduct() {
        ...
        Utils.initListView(this, binding.ppggoodsListRecycler, new DividerDecoration(Color.parseColor("#f6f6f6"), 0), adapter, new PullListener() {
            @Override
            ...
            public void onLoadMore() {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        binding.ppggoodsListRecycler.setRefreshing(false);
                        if (page == 1) {
                            return;
                        }
                        viewModel.getList(page);
